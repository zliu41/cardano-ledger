...
\begin{itemize}
  ...
  script.
  \item $\fun{exunits}$ calculates the total $\ExUnits$ in a transaction.
...
\begin{itemize}
\item
  $\ScriptPurpose$ captures the exact purpose a script is used
  for, for example spending a specific output or minting with a
  specific policy.
\item
...
\begin{figure}[htb]
  \begin{align*}
    ...
    & ~~~~~~~ \var{txb}~=~\txbody{tx}
    \nextdef
    & \hspace{-1cm}\fun{checkRedeemer} \in \Tx \to (\ScriptPurpose \times \ScriptHash) \to \Bool \\
    & \hspace{-1cm}\fun{checkRedeemer}~\var{tx}~(\var{sp},\var{h})~=~ \exists s, (h\mapsto s) \in \fun{txscripts}~(\fun{txwits}~tx)\\
...
\begin{figure}
  \begin{equation}
    ...
    \inference[UTxO-witG]
    {
      ...
      \dom (\txwitsVKey{txw}) \}\\
      \hldiff{\var{extraKeyHashes} \leteq \{\fun{hashKey}~\var{vk} \vert \var{vk} \in
      \fun{extraKeys}~txb \}}\\~\\
      \hldiff{\forall \var{s} \in \range (\fun{txscripts}~{txw}) \cap \ScriptNative,
      \fun{runNativeScript}~\var{s}~\var{tx}}\\~\\
      \hldiff{\forall sph \in \fun{scriptsNeeded}~\var{utxo}~\var{tx}, \fun{checkRedeemer}~tx~sph} \\
      ...
      \fun{witsVKeyNeeded}~{utxo}~{tx}~{genDelegs} \cup \hldiff{extraKeyHashes} \subseteq \var{witsKeyHashes}
      \\~\\
      genSig \leteq
      \left\{
