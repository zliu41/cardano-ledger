...
--------------------------------------------------------------------------------
-- Copy from Cardano.Ledger.ShelleyMA.TxBody
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
-- END Copy from Cardano.Ledger.ShelleyMA.TxBody
--------------------------------------------------------------------------------
...
-- TxBody has three Era dependent type families (Value era), (AuxiliaryData
-- era), and (Script era) (hidden in DCert) in order to make CBOR instances of
-- things we are going to have to assume some properties about these.
...
pattern TxBody ::
  ...
  Coin ->
  ValidityInterval ->
  StrictMaybe (Update.Payload era) ->
...
instance HasField "update" (TxBody era) (StrictMaybe (Update.Payload era)) where
...
ppTxBody ::
  ...
  TxBody era ->
  PDoc
ppTxBody (TxBodyConstr (Memo (TxBodyRaw i o d w fee vi _u m mint) _)) =
  ppRecord
    ...
    [ ("inputs", ppSet ppTxIn i),
      ...
      ("update", "todo: pretty print the update payload if you need this"),
