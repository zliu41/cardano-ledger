...
module Test.Shelley.Spec.Ledger.Utils2
  ( assertAll,
    mkSeedFromWords,
    mkCertifiedVRF,
    epochFromSlotNo,
...
import Cardano.Crypto.DSIGN (deriveVerKeyDSIGN, genKeyDSIGN, SignKeyDSIGN)
...
import Cardano.Crypto.KES (deriveVerKeyKES, genKeyKES, SignKeyKES, VerKeyKES, KESAlgorithm)
...
import Cardano.Prelude (asks, Coercible)
...
type VKeyGenesis c = VKey 'Genesis c
...
mkCertifiedVRF :: (VRF.Signable v a, VRFAlgorithm v,ContextVRF v ~ (), Coercible b (CertifiedVRF v a)) => a -> SignKeyVRF v -> b
mkCertifiedVRF a sk =
