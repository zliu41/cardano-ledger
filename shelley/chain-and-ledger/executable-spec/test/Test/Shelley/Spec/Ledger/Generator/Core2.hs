...
module Test.Shelley.Spec.Ledger.Generator.Core2
  ( AllIssuerKeys (..),
    applyTxBody,
    GenEnv (..),
    KeySpace (..),
...
import Shelley.Spec.Ledger.Address (Addr, toAddr, toCred, pattern Addr)
...
import Shelley.Spec.Ledger.OCert (OCert, KESPeriod (..), OCertSignable (..), pattern OCert)
...
import Test.Shelley.Spec.Ledger.Utils2
  ( epochFromSlotNo,
    evolveKESUntil,
    maxKESIterations,
    maxLLSupply,
...
deriving instance (Crypto c,HashAlgorithm c) => Show (KeySpace c)


pattern KeySpace ::
  (Crypto h,HashAlgorithm h) =>
...
lookupGenDelegate ::
  (Crypto crypto,HashAlgorithm crypto) =>
...
mkKeyPairs :: (HasCallStack,DSIGNAlgorithm (DSIGN kr),DSIGNAlgorithm (DSIGN kr')) =>
              Word64 -> (KeyPair h kr, KeyPair h kr')
mkKeyPairs n =
...
mkStakeKeyHashMap :: (HasCallStack, Crypto c,HashAlgorithm c) => KeyPairs c -> Map (KeyHash 'Staking c) (KeyPair 'Staking c)
...
mkPayKeyHashMap :: (HasCallStack, Crypto c, HashAlgorithm c) =>
...
mkMSigScripts :: (HasCallStack, Crypto c, HashAlgorithm c) => KeyPairs c -> MultiSigPairs c
...
mkMSigCombinations :: (HasCallStack, Crypto c, HashAlgorithm c) => MultiSigPairs c -> MultiSigPairs c
...
mkScriptsFromKeyPair ::
  (HasCallStack, Crypto c,HashAlgorithm c) =>
...
mkScriptFromKey :: (HasCallStack, Crypto c, HashAlgorithm c) => KeyPair 'Witness c -> MultiSig c
...
findPayScriptFromCred :: (HasCallStack, Crypto c, HashAlgorithm c) =>
                         Credential 'Witness c -> MultiSigPairs c -> (MultiSig c, MultiSig c)
...
-- | Find first matching script for a credential.
findStakeScriptFromCred :: (HasCallStack, Crypto c, HashAlgorithm c) =>
                           Credential 'Witness c -> MultiSigPairs c -> (MultiSig c, MultiSig c)
...
findPayScriptFromAddr :: (HasCallStack, Crypto c, HashAlgorithm c) => Addr c -> MultiSigPairs c -> (MultiSig c, MultiSig c)
...
genTxOut :: (HasCallStack, Crypto c, HashAlgorithm c) => Constants -> [Addr c] -> Gen [TxOut c]
...
mkOCert ::
  forall h r.
  (HasCallStack, Crypto h, HashAlgorithm h, Signable (DSIGN h) (OCertSignable h)) =>
  AllIssuerKeys h r ->
  Word64 ->
  KESPeriod ->
  OCert h
mkOCert pkeys n c0 =
  let (_, (_, vKeyHot)) = head $ hot pkeys
   ...
   in OCert
        ...
        n
        c0
        (signedDSIGN @h sKeyCold (OCertSignable vKeyHot n c0))
