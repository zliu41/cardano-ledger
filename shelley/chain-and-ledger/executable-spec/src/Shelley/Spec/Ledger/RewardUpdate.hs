...
-- This type was previously just a tuple, so we directly reuse that semantics
-- to be explicit.
instance CC.Crypto c => ToCBOR (RewardAns c) where
  toCBOR (RewardAns x y) = toCBOR (x, y)

  encodedSizeExpr size = encodedSizeExpr size . fmap (\(RewardAns x y) -> (x, y))
  encodedListSizeExpr size = encodedListSizeExpr size . fmap (map (\(RewardAns x y) -> (x, y)))
